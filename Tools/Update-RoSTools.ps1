...
# Everything below runs inside a block so Set-StrictMode / $ErrorActionPreference
# and all the helper functions/state stay scoped to this invocation. Without this,
# running the script via `irm ... | iex` executes it directly in the caller's
# session -- there's no script file to give it its own scope -- so StrictMode and
# EAP=Stop leak out and stick around, breaking prompt functions and profile code
# that rely on default (non-strict, non-terminating) behavior.
& {
    Set-StrictMode -Version Latest
    $ErrorActionPreference = 'Stop'

    # State lives beside the user's other app data, not in the repo.
    $StateDir  = Join-Path $env:LOCALAPPDATA 'RoS-Tools'
    $StateFile = Join-Path $StateDir 'updater-state.json'
    ...
    try {
        ...
        if ((Test-Path variable:destination) -and $destination -and (Test-Path $destination)) {
