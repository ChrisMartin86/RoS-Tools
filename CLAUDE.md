...
`Riddled` — a World of Warcraft **retail** addon (Interface 120000) that shows
...
- **`Riddled.toc` load order is significant and manual.** Any new file must be
...
- **SavedVariables (`RiddledDB`) hold settings only.** The ilvl table is rebuilt
...
(`local _, ns = ...`). No globals except `RiddledDB` and the slash-command
plumbing.
...
add it to `Riddled.toc` *above* `Core/Events.lua`.
...
character's ilvl never resolves, check that table first.** `/riddle set debug`
...
- New settings: add to `DEFAULTS` in `Config.lua` — `/riddle set` and the
...
`World of Warcraft\_retail_\Interface\AddOns\Riddled` (the `.toc` sits directly
inside), then `/reload`.
